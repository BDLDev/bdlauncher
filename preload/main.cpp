...
void register_shutdown(void *fn) { onsht.push_back(fn); }
void set_int_handler(void *fn) { int_handler = (typeof(int_handler)) fn; }
void call_sht(int d) {
  for (auto i : onsht) { ((void (*)(int)) i)(d); }
  if (d == SIGINT) {
    if (int_handler) {
      int_handler();
    } else {
...
#define sh(a) signal(a, call_sht);
  sh(SIGHUP) sh(SIGINT) sh(SIGILL) sh(SIGABRT) sh(SIGFPE) sh(SIGSEGV)
}
