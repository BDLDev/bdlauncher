...
bool isOre(short x) { return ores.count(x); }
...
void reCalcChunk(shared_ptr<LevelChunk> chunk, unordered_set<int> &l) {
  ...
  for (int k = 1; k <= 64; ++k)
    for (int i = 0; i < 16; ++i)
      for (int j = 0; j < 16; ++j) {
        ids[k][i + 1][j + 1] =
            chunk->getBlock({(unsigned char) i, (short) k, (unsigned char) j})->getLegacyBlock()->getBlockItemId();
      }
  for (int k = 1; k <= 64; ++k)
    for (int i = 1; i <= 16; ++i)
      for (int j = 1; j <= 16; ++j) {
        if (isOre(ids[k][i][j])) {
          int fg = ids[k + 1][i][j] == 0 || ids[k - 1][i][j] == 0 || ids[k][i + 1][j] == 0 || ids[k][i - 1][j] == 0 ||
                   ids[k][i][j + 1] == 0 || ids[k][i][j - 1] == 0;
          if (!fg) {
...
void cache_push(unsigned long hash, unordered_set<int> *v) {
  ...
  if (anti_cache.size() >= 128) {
    auto x = anti_cache.back().second;
    delete x;
    anti_cache.pop_back();
  }
}
void calcChunk(
    shared_ptr<LevelChunk> chunk, ChunkPos &cpos, NetworkChunkPublisher &thi, list<std::pair<int, Block *>> &undolist) {
...
void undoChunk(shared_ptr<LevelChunk> chunk, list<std::pair<int, Block *>> &ores) {
  for (auto &i : ores) { chunk->setBlock2(fcast(ChunkBlockPos, i.first), *i.second); }
}
THook(
