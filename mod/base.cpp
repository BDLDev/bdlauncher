...
bool give_item(Player* a,const string& name,short spec,int amo){
    ItemStack sk(Item(name,spec),amo);
    return ((PlayerInventoryProxy*)a->getSupplies())->add(sk,true);
}
bool red_item(Player* a,const string& name,short spec,int amo){
    ItemStack sk(Item(name,spec),amo);
    return ((PlayerInventoryProxy*)a->getSupplies())->removeResource(sk);
}
...
void base_init(list<string>& modlist)
{
       ...
       // void* tpco=fp(dlsym(RTLD_DEFAULT,"_ZN15TeleportCommandC2Ev"));
       // printf("%p\n",tpco);
       // tcmd=(typeof(tcmd))MyHook(tpco,fp(hktp));
       // csi=(typeof(csi))MyHook(fp(dlsym(NULL,"_ZN12ServerPlayer11slotChangedER17IContainerManageriRK9ItemStackS4_b")),fp(hk));
