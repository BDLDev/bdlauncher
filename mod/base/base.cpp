...
THook(void *, _ZN14ServerInstance14onLevelCorruptEv, void *x) {
  ...
  for (auto &p : *vc) {
    auto sp = getSP(p.get());
    if (sp) {
      payload += sp->getNameTag() + " ->(" + std::to_string(sp->getPos().x) + " , " + std::to_string(sp->getPos().z) +
                 ") dim " + std::to_string(sp->getDimensionId()) + "\n";
    }
  }
  printf("%s\n", payload.c_str());
  char buf[1024];
...
BDL_EXPORT void regSPD2EvHooks(pair<void (*)(void *, ServerPlayer *), void *> x) { SPD2EvHooks.push_back(x); }
THook(void *, _ZN12ServerPlayerD2Ev, ServerPlayer *thi) {
  for (auto &i : SPD2EvHooks) { i.first(i.second, thi); }
  return original(thi);
}
static void dummy__() {}
