...
BDL_EXPORT void register_cmd(
    const std::string &name, void (*fn)(argVec &, CommandOrigin const &, CommandOutput &outp, string_view),
    const std::string &desc, int perm) {
  reqs.push_back({name, desc, perm, fp(fn)});
}
BDL_EXPORT void register_cmd(
    const std::string &name, void (*fn)(argVec &, CommandOrigin const &, CommandOutput &outp), const std::string &desc,
    int perm) {
  reqs.push_back({name, desc, perm, fp(fn)});
}
