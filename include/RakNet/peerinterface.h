...
public:
  ...
  /// \param[in] maxConnections The maximum number of connections between this instance of RakPeer and another instance of RakPeer. Required so the network can preallocate and for thread safety. A
  ...
  /// can overrun the available bandwidth. \param[in] timeoutTime How long to keep the connection alive before dropping it on unable to send a reliable message. 0 to use the default from
  ...
  virtual ConnectionAttemptResult Connect(const char *host, unsigned short remotePort, const char *passwordData, int passwordDataLength, PublicKey *publicKey = 0, unsigned connectionSocketIndex = 0,
                                          unsigned sendConnectionAttemptCount = 12, unsigned timeBetweenSendConnectionAttemptsMS = 500, TimeMS timeoutTime = 0) = 0;
  ...
  /// \param[out] remoteSystems An array of SystemAddress structures to be filled with the SystemAddresss of the systems we are connected to. Pass 0 to remoteSystems to only get the number of systems
  /// we are connected to \param[in, out] numberOfSystems As input, the size of remoteSystems array.  As output, the number of elements put into the array
  ...
  virtual uint32_t Send(const char *data, const int length, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast,
  ...
  virtual uint32_t Send(const BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast,
                        uint32_t forceReceiptNumber = 0) = 0;
  ...
  /// \return 0 on bad input. Otherwise a number that identifies this message. If \a reliability is a type that returns a receipt, on a later call to Receive() you will get ID_SND_RECEIPT_ACKED or
  /// ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive containing this number
  virtual uint32_t SendList(const char **data, const int *lengths, const int numParameters, PacketPriority priority, PacketReliability reliability, char orderingChannel,
  ...
  virtual void AddToBanList(const char *IP, TimeMS milliseconds = 0) = 0;
  ...
  /// Send a ping to the specified unconnected system. The remote system, if it is Initialized, will respond with ID_PONG followed by sizeof(TimeMS) containing the system time the ping was
  /// sent.(Default is 4 bytes - See __GET_TIME_64BIT in types.h System should reply with ID_PONG if it is active \param[in] host Either a dotted IP address or a domain name.  Can be 255.255.255.255
  /// for LAN broadcast. \param[in] remotePort Which port to connect to on the remote machine. \param[in] onlyReplyOnAcceptingConnections Only request a reply if the remote system is accepting
  /// connections \param[in] connectionSocketIndex Index into the array of socket descriptors passed to socketDescriptors in RakPeer::Startup() to send on. \return true on success, false on failure
  /// (unknown hostname)
  ...
  virtual Time GetClockDifferential(const AddressOrGUID systemIdentifier) = 0;
  ...
  /// MAXIMUM_NUMBER_OF_INTERNAL_IDS-1) \return the identifier of your system internally, which may not be how other systems see if you if you are behind a NAT or proxy
  virtual SystemAddress GetInternalID(const SystemAddress systemAddress, const int index) const = 0;
  ...
  virtual TimeMS GetTimeoutTime(const SystemAddress target) = 0;
  ...
  virtual void SetUnreliableTimeout(TimeMS timeoutMS) = 0;
  ...
  /// \return A smart pointer object containing the socket information about the socket. Be sure to check IsNull() which is returned if the update thread is unresponsive, shutting down, or if this
