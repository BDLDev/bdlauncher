...
#include <fcntl.h>
using std::string;
template <typename T> static auto autosize(const T &a) { return std::distance(a.begin(), a.end()); }
...
template <class MP, typename T> static int settomem(MP &mp, char **mem, string (*val)(const T &)) {
  char *buf = (char *) malloc(64 * 1024 * 1024);
  int ptr   = 0;
...
template <class MP, typename TT> static int memtoset(MP &mp, char *buf, void (*val)(const string &, TT &)) {
  int ptr = 0;
  int sz;
  getint(buf, ptr, sz);
  TT v;
...
static string h_str2str(const string &v) { return v; }
static void h_str2str_load(const string &v, string &vv) { vv = v; }
